...
class UserSolution {
	...
	final int MAX_NODE = MAX_LOAD * 2 + MAX_CITY * MAX_CITY;
	...
	public UserSolution() {
		...
		this.nodePool = new Node[MAX_NODE];
	...
	int dijkstra(int mHub, ArrayList<Node>[] load, int[] distance) {
		...
		queue.clear();
		queue.offer(this.nodePool[++this.nodePoolIdx].set(mHub, 0));
		...
		while(!queue.isEmpty()) {
			...
			for (Node next : load[node.idx]) {
				if(distance[next.idx] > node.weight + next.weight) {
					...
					queue.offer(this.nodePool[++this.nodePoolIdx].set(next.idx, distance[next.idx]));
				}
			}
		}
