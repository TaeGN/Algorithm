...
public class UserSolution_pass
{
	...
	private final int MAX_M;
	private final int MAX_M_MAP;
	private final int MAX_M_STRUCTURE;
	private final int MAX_CANDIDATE;
	...
	private final Map<Integer, List<Candidate>> candidateMap;
	...
	public int maxArea(int M, int mStructure[], int mSeaLevel)
	{
		...
		List<Candidate> candidateList = candidateMap.get(candidateKey);
		for (int i = 0; i < len; i++) {
			mMap = copyMap();
			queue.clear();
