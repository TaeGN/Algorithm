...
public class Main_전투시뮬레이션_print {
...
//			unitList[i] = new Unit(readInt(input[0]), readInt(input[1]), readInt(input[2]), readInt(input[3]));
			unitList[i] = new Unit(readInt(input[0]), readInt(input[1]), readInt(input[2]), readInt(input[3]), i);
...
//		for (int i = 0; i < H; i++) {
//			for (int j = 0; j < W; j++) {
//				if(unitMap[i][j] == null) sb.append(0);
//				else sb.append(unitMap[i][j].idx);
//				sb.append(" ");
//			}
//			sb.append("\n");
//		}
...
//	}
	...
	static class Unit {
		...
		public Unit(int mobility, int team, int r, int c, int idx) {
			super();
			this.mobility = mobility;
			this.team = team;
			this.r = r;
		...
		public boolean move(int er, int ec) {
			...
			int c = this.c;
			// 이동 불가 지형 제외
			if(map[er][ec] > this.mobility) return false;
			// dfs
