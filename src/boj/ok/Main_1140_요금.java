...
public class Main_1140_요금 {
	...
	private static long getMinCharge(long T, long K1, long P1, long K2, long P2) {
		...
		long min = P1 * (cnt1 + 1);
		long cnt2 = T / K2;
		min = Math.min(min, P2 * (cnt2 + 1));
		long len = K2 / mcf;
		for(int i = 0; i < len; i++) {
			min = Math.min(min, P1 * cnt1 + Math.min((((T - K1 * cnt1) / K2) + 1) * P2, ((T - K1 * cnt1) / K2) * P2 + ((T - K1 * cnt1) % K2) * 10));
			min = Math.min(min, P2 * cnt2 + Math.min((((T - K2 * cnt2) / K1) + 1) * P1, ((T - K2 * cnt2) / K1) * P1 + ((T - K2 * cnt2) % K1) * 10));
			cnt1--; cnt2--;
		}
