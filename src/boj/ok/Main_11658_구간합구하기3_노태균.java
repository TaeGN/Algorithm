...
public class Main_11658_구간합구하기3_노태균 {
	static int N, M;
	static long tree[][];
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < M; i++) {
			...
			if(input[0].equals("0")) update(tree[r1], c1, Integer.parseInt(input[3]) - (int)lineSum(r1, c1, c1));
			else {
	...
	static void print() {
		for (int i = 1; i <= N; i++) {
			for (int j = 1; j <= N; j++) {
				System.out.print(sectionSum(i, j, i, j) + " ");
			}
			System.out.println();
		}
	}
	...
	static long lineSum(int r, int c1, int c2) {
