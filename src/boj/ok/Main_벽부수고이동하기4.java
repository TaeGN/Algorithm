...
public class Main_벽부수고이동하기4 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < M; j++) {
				if(map[i][j] == WALL) {
					sb.append(getMoveCnt(i, j, moveCnt, map, selected));
				} else {
					sb.append(0);
				}
			}
	...
	static int getMoveCnt(int r, int c, List<Integer> moveCnt, int[][] map, int[] selected) {
		int cnt = 1;
		...
		a:for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < idx; j++) {
				if(map[rr][cc] == selected[j]) continue a;
			}
			selected[idx++] = map[rr][cc];
