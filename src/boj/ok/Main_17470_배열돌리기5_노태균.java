...
public class Main_17470_배열돌리기5_노태균 {
	static int N, M, R;
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				arr[i][j] = new Point(i, j, 0);
			}
		}
		for (int i = 0; i < N; i++) {
			input = br.readLine().split(" ");
			for (int j = 0; j < M; j++) {
				arr2[i][j] = new Point(i, j, Integer.parseInt(input[j]));
			}
		}
		...
		if(prev != null) calculation(prev, "");
		
		if(reversalUD == 1) rotate(arr2, 1);
		if(reversalLR == 1) rotate(arr2, 2);
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < M; j++) {
				result[arr2[i][j].r][arr2[i][j].c] = arr2[i][j].v;
			}
		}
	...
	static String calculation(String prev, String next) {
		...
		case "15":case "51":
			return "6";
		default:
			rotate(arr, Integer.parseInt(prev));
			return next;
		}
	}
	
	static void rotate(Point[][] arr, int num) {
	...
	static class Point {
		...
		public String toString() {
			...
			builder.append("Point [r=").append(r).append(", c=").append(c).append(", v=").append(v).append("]");
